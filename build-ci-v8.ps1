...
Write-Host "==============================================="
Write-Host "V8 - FORCE COMPILER INCLUDE PATH"
Write-Host "==============================================="
...
# Make ATL + WTL visible to every project in the SDK tree, including
# libPPUI and foobar2000_sdk_helpers. Directory.Build.props is imported
# automatically by MSBuild for projects below this directory.
$propsPath = Join-Path $SdkRoot "Directory.Build.props"
$escapedAtl = [System.Security.SecurityElement]::Escape($AtlInclude)
$escapedWtl = [System.Security.SecurityElement]::Escape($WtlInclude)

$props = @"
<Project>
  <ItemDefinitionGroup>
    <ClCompile>
      <AdditionalIncludeDirectories>$escapedAtl;$escapedWtl;%(AdditionalIncludeDirectories)</AdditionalIncludeDirectories>
    </ClCompile>
  </ItemDefinitionGroup>
</Project>
"@
...
Write-Host "Created global SDK props:"
...
try {
    ...
    # C++17 for std::filesystem, plus WTL include path.
    foreach ($idg in @($xml.SelectNodes("//m:ItemDefinitionGroup", $ns))) {
        ...
        $inc = $cl.SelectSingleNode("m:AdditionalIncludeDirectories", $ns)
        if (-not $inc) {
            $inc = $xml.CreateElement("AdditionalIncludeDirectories", $nsUri)
            [void]$cl.AppendChild($inc)
            $inc.InnerText = "$WtlInclude;%(AdditionalIncludeDirectories)"
        }
        elseif ($inc.InnerText -notlike "*$WtlInclude*") {
            $inc.InnerText = "$WtlInclude;" + $inc.InnerText
        }
    }
    ...
    # Force ATL + WTL into the environment seen by every cl.exe invocation.
    # This reaches SDK dependency projects even if they do not import
    # Directory.Build.props.
    $oldInclude = $env:INCLUDE
    if ([string]::IsNullOrWhiteSpace($oldInclude)) {
        $env:INCLUDE = "$AtlInclude;$WtlInclude"
    } else {
        $env:INCLUDE = "$AtlInclude;$WtlInclude;$oldInclude"
    }

    Write-Host "Compiler INCLUDE begins with:"
    Write-Host "$AtlInclude;$WtlInclude"
    ...
    # The SDK projects build to their normal individual output directories.
    # Only foo_sample was patched to use the final component target name.
    $builtDll = Get-ChildItem -Path $sampleDir -Filter "foo_stem_separator.dll" -Recurse |
        Select-Object -First 1

    if (-not $builtDll) {
        # Fallback: look throughout the extracted SDK tree.
        $builtDll = Get-ChildItem -Path $SdkRoot -Filter "foo_stem_separator.dll" -Recurse |
            Select-Object -First 1
    }

    if (-not $builtDll) {
        Write-Host "DLL files found under the sample directory:"
        Get-ChildItem -Path $sampleDir -Filter "*.dll" -Recurse -ErrorAction SilentlyContinue |
